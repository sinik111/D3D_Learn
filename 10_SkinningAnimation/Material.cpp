...
namespace
{
	TextureSRVs* g_defaultTextureSRVs = nullptr;
}
...
Material::Material(const Microsoft::WRL::ComPtr<ID3D11Device>& device, aiMaterial* material)
{
	...
	else if (aiReturn_SUCCESS == material->Get(AI_MATKEY_COLOR_DIFFUSE, aiColor))
	{
		unsigned char color[4]{
			...
			static_cast<unsigned char>(aiColor.g * 255),
			static_cast<unsigned char>(aiColor.b * 255),
			static_cast<unsigned char>(aiColor.a * 255),
		};
		m_textureSRVs.diffuseTextureSRV = CreateDiffuseTextureSRV(device, color);
	}
	else
	{
		m_textureSRVs.diffuseTextureSRV = g_defaultTextureSRVs->diffuseTextureSRV;
	}
	...
	{
		m_textureSRVs.normalTextureSRV = g_defaultTextureSRVs->normalTextureSRV;
	}
	...
	{
		m_textureSRVs.specularTextureSRV = g_defaultTextureSRVs->specularTextureSRV;
	}
	...
	{
		m_textureSRVs.emissiveTextureSRV = g_defaultTextureSRVs->emissiveTextureSRV;
	}
	...
	{
		m_textureSRVs.opacityTextureSRV = g_defaultTextureSRVs->opacityTextureSRV;
	}
}
...
{
	if (g_defaultTextureSRVs != nullptr)
	{
		return;
	}
	...
	g_defaultTextureSRVs = new TextureSRVs;
	...
	for (int i = 0; i < TextureType::Max; ++i)
	{
		...
		{
		case Diffuse:
			device->CreateShaderResourceView(texture.Get(), &srvDesc, &g_defaultTextureSRVs->diffuseTextureSRV);
			break;
		case Normal:
			device->CreateShaderResourceView(texture.Get(), &srvDesc, &g_defaultTextureSRVs->normalTextureSRV);
			break;
		case Specular:
			device->CreateShaderResourceView(texture.Get(), &srvDesc, &g_defaultTextureSRVs->specularTextureSRV);
			break;
		case Emissive:
			device->CreateShaderResourceView(texture.Get(), &srvDesc, &g_defaultTextureSRVs->emissiveTextureSRV);
			break;
		case Opacity:
			device->CreateShaderResourceView(texture.Get(), &srvDesc, &g_defaultTextureSRVs->opacityTextureSRV);
			break;
		}
	}
}
	...
	const Microsoft::WRL::ComPtr<ID3D11Device>& device, unsigned char color[4])
{
	...
	device->CreateShaderResourceView(texture.Get(), &srvDesc, &diffuseTextureSRV);

	return diffuseTextureSRV;
}

void Material::DestroyDefaultTextureSRV()
{
	delete g_defaultTextureSRVs;

	g_defaultTextureSRVs = nullptr;
}

const TextureSRVs& Material::GetDefaultTextureSRVs()
{
	return *g_defaultTextureSRVs;
}