...
void VertexBuffer::Create(const Microsoft::WRL::ComPtr<ID3D11Device>& device, const std::vector<PositionNormalVertex3D>& vertices)
{
	m_bufferStride = sizeof(PositionNormalVertex3D);

	D3D11_BUFFER_DESC vertexBufferDesc{};
	vertexBufferDesc.ByteWidth = static_cast<UINT>(sizeof(PositionNormalVertex3D) * vertices.size());
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
...
void VertexBuffer::Create(const Microsoft::WRL::ComPtr<ID3D11Device>& device, const std::vector<PositionVertex3D>& vertices)
{
	m_bufferStride = sizeof(PositionVertex3D);

	D3D11_BUFFER_DESC vertexBufferDesc{};
	vertexBufferDesc.ByteWidth = static_cast<UINT>(sizeof(PositionVertex3D) * vertices.size());
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
